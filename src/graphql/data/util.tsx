...
export function gqlToCurrency(token: {
  ...
  if (token.standard === TokenStandard.Native || token.address === 'NATIVE' || !token.address) { return nativeOnChain(chainId); }
...
export function logSentryErrorForUnsupportedChain({ extras,
  errorMessage }: {
...
export function getTokenDetailsURL({ address,
  chain,
  inputAddress,
