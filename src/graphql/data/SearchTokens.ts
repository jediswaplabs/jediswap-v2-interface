...
function dedupeCrosschainTokens(current: SearchToken, existing: SearchToken | undefined, searchChain: Chain) {
  ...
  if (current.chain === searchChain || (existing.chain !== searchChain && current.chain === Chain.Ethereum)) { return current; }
...
function searchTokenSortFunction(
  ...
  if (a.standard === 'NATIVE') {
    if (b.standard === 'NATIVE') {
      ...
      if (b.chain === searchChain) { return 1; }
      return 0;
    } return -1;
  } if (b.standard === 'NATIVE') { return 1; }
  if (wrappedNativeAddress && a.address === wrappedNativeAddress) { return -1; }
  if (wrappedNativeAddress && b.address === wrappedNativeAddress) { return 1; }
...
export function useSearchTokens(searchQuery: string, chainId: number) {
  ...
  const sortedTokens = useMemo(() => {
    ...
    const filteredTokens = data?.searchTokens?.filter((token) => (BACKEND_SUPPORTED_CHAINS as ReadonlyArray<Chain>).includes(token.chain));
