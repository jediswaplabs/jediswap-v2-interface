...
import { Pool, Token, usePoolDataQuery } from './types-and-hooks';
...
export function usePoolData(
  ...
  const { loading: loading24,
    error: error24,
    data: data24 } = usePoolDataQuery({
    variables: { poolId, block: { number: parseFloat(block24?.number) } },
    client: apolloClient,
    fetchPolicy: 'no-cache',
  });
  const { loading: loading48,
    error: error48,
  ...
  const { loading: loadingWeek,
    error: errorWeek,
    data: dataWeek } = usePoolDataQuery({
