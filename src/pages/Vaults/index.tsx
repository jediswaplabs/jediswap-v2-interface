...
const ListItem = ({ index, vaultAddress, vaultData, getUserBalance = noop }) => {
  ...
  if (!(vaultData.token0 && vaultData.token1 && shareTokenAddress)) { return null; }
    ...
    <DashGrid style={{ height: '48px' }}>
      ...
      <DataText area="deposite">
        <ThemedText.BodySmall>
          <UserBalance vaultAddress={vaultAddress} tokenAddress={shareTokenAddress} tokenPrice={shareTokenPriceUsd} getResult={getUserBalance} />
        </ThemedText.BodySmall>
      </DataText>
    </DashGrid>
  );
...
export default function Vaults({ maxItems = 10 }) {
  ...
  useEffect(() => {
    if (!isConnected) { setIsMyVaultsFilterEnabled(false); }
    setUserPools({});
  }, [isConnected]);
  ...
  const vaultsList = vaultsAddresses
    ...
    .sort((vaultAddressA, vaultAddressB) => {
      ...
      if (vaultAValueToCompare < vaultBValueToCompare) { return 1; }
      if (vaultAValueToCompare > vaultBValueToCompare) { return -1; }
      return 0;
    });
