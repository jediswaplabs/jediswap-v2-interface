...
export const reduceFilters = (state: typeof initialFilterState, action: { eventType: ActivityEventType }) => ({ ...state, [action.eventType]: !state[action.eventType] });
...
export const Activity = ({ contractAddress, rarityVerified, collectionName, chainId }: ActivityProps) => {
  ...
  const { nftActivity,
    ...
    loading: activitiesAreLoading } = useNftActivity(
    {
