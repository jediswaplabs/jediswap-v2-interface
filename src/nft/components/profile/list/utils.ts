...
import { LOOKSRARE_MARKETPLACE_CONTRACT_721,
  LOOKSRARE_MARKETPLACE_CONTRACT_1155 } from 'nft/queries/looksRare/constants';
...
export async function approveCollectionRow(
  ...
  const spender = marketplace.name === 'OpenSea'
...
export async function signListingRow(
  ...
  const callback = () => signListingRow(listing, signer, provider, getLooksRareNonce, setLooksRareNonce, setListingStatusAndCallback);
  setListingStatusAndCallback(listing, ListingStatus.SIGNING, callback);
  const { asset, marketplace } = listing;
  const res = await signListing(marketplace, asset, signer, provider, looksRareNonce, (newStatus: ListingStatus) => setListingStatusAndCallback(listing, newStatus, callback));
...
export const getTotalEthValue = (sellAssets: WalletAsset[]) => {
  const total = sellAssets.reduce((total, asset: WalletAsset) => {
    if (asset.newListings?.length) {
      ...
      const maxFee = maxListing.marketplace.fee
...
export const verifyStatus = (status: ListingStatus) => status !== ListingStatus.PAUSED && status !== ListingStatus.APPROVED;
...
export function useUpdateInputAndWarnings(
  ...
  useEffect(() => {
    ...
    if (price < (asset?.floorPrice ?? 0) && price > 0) { setWarningType(WarningType.BELOW_FLOOR); } else if (
...
export const getRoyalty = (listingMarket: ListingMarket, asset: WalletAsset) => {
  ...
  const baseFee = listingMarket.name === 'LooksRare'
    ? asset.basisPoints
...
export const findListingIssues = (sellAssets: WalletAsset[]) => {
  ...
  const overMaxExpiration = sellAssets.some((asset) => asset.expirationTime != null && asset.expirationTime * 1000 - Date.now() > ms('180d'));
  ...
  for (const asset of sellAssets) {
    if (asset.newListings) {
      for (const listing of asset.newListings) {
        if (!listing.price) { listingsMissingPrice.push([asset, listing]); } else if (listing.price < (asset?.floorPrice ?? 0) * BELOW_FLOOR_PRICE_THRESHOLD && !listing.overrideFloorPrice) { listingsBelowFloor.push([asset, listing]); } else if (
          asset.floor_sell_order_price
