...
export async function getApproveInfo(
  ...
  const tokenContract = provider && getContract(currency.address, ERC20_ABI, provider)
  ...
  return { needsApprove: true, approveGasEstimateUSD: 1 }
}
...
export async function getWrapInfo(
  ...
  return { needsWrap: true, wrapGasEstimateUSD: 1 }
}
