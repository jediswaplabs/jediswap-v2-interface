...
export function useAllVaults() {
  ...
  const loadData = async () => {
    ...
    try {
      const [vaultListWithContents, permissionlessVaultDataList] = await Promise.all([getVaultListWithContents(), getPermissionlessVaultDataList()]);
      const addresses = Object.keys(vaultListWithContents);
      const tokensAddresses = uniq(addresses.flatMap((address) => [vaultListWithContents[address]?.token0?.address, vaultListWithContents[address]?.token1?.address]).filter(Boolean));
      const prices = await getTokenPrices(tokensAddresses);
      const combinedData = addresses.reduce((acc, address) => {
        if (!permissionlessVaultDataList[address]) { return acc; }
