...
import { themeBorderGradient } from '../styles';
 ...
 */
export function ExternalLink({ target = '_blank',
  href,
  rel = 'noopener noreferrer',
  ...rest }: Omit<HTMLProps<HTMLAnchorElement>, 'as' | 'ref' | 'onClick'> & { href: string }) {
...
const ToolTipWrapper = styled.div<{ isCopyContractTooltip?: boolean; tooltipX?: number }>`
  ...
  align-items: center;
  position: absolute;
  left: ${({ isCopyContractTooltip, tooltipX }) => isCopyContractTooltip && (tooltipX ? `${tooltipX - TOOLTIP_WIDTH / 2}px` : '50%')};
...
export const Separator = styled.div<{ size?: number }>`
  width: 100%;
  height: ${({ size }) => (size ? `${size}px` : '1px')};
  background-color: ${({ theme }) => theme.surface5};
`;

export const ThemedSeparator = styled.div<{ reversed?: boolean }>`
  width: 100%;
  ${({ theme, reversed }) => themeBorderGradient({
    gradientColor: (reversed) ? theme.brandedGradientReversed : theme.brandedGradient,
    size: 0,
  })};
