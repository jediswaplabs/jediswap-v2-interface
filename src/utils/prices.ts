...
import { TokenAmount, CurrencyAmount as CurrencyAmountV1 } from '@jediswap/sdk'
...
// computes price breakdown for the trade
export function computeTradePriceBreakdown(trade): { priceImpactWithoutFee?: Percent; realizedLPFee?: CurrencyAmountV1 } {
  // for each hop in our trade, take away the x*y=k price impact from 0.3% fees
  // e.g. for 3 tokens/2 hops: 1 - ((1 - .03) * (1-.03))
  if (trade) {
  }
  ...
  const realizedLPFeeAmount = realizedLPFee
      ...
      && (trade.inputAmount instanceof TokenAmount
        ...
        : CurrencyAmountV1.ether(realizedLPFee.multiply(trade.inputAmount.raw).quotient))
